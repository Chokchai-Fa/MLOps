pipeline {
    ...
    stages{
        ...
        stage('PreProcessing Stage'){
            steps{
                script{
                  sh 'docker exec mlops-test  python3 preprocessing.py'
                }
            } 
            }
        ...
        stage('Train Stage'){
            steps{
                script{
                  sh 'docker exec mlops-test  python3 train.py'
                }
            } 
            }
        ...
        steps{
            script{
                sh 'docker exec mlops-test  python3 test.py'
                sh 'docker exec mlops-test  cat /results/train_metadata.json /results/test_metadata.json'
            }
        } 
        }
